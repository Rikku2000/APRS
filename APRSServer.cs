...
namespace APRSWebServer
{
    ...
    {
        public static string Build = "HTTP";
        ...
        private void InitConfig()
        {
            ...
            if ((nl.Count > 0) && OutConfigToConsole) Console.WriteLine("Loading config from `aprs.xml`...");
        ...
        protected override void GetClient(TcpClient Client, ulong clientID)
        {
            ...
            while ((bRead = Client.GetStream().ReadByte()) >= 0)
            {
                ...
                if (posCRLF >= 0 || receivedText.Length > 2048) { break; };
            };
        ...
        private void OnAPRSData(ClientData cd, string line)
        {
            ...
            if (cd.lastBuddie == null) return;        
            if ((!cd.validated) && (!cd.lastBuddie.PositionIsValid)) return;
