        ...
        public class F_t : F
        {
            ...
            public override bool Pass(string APRS, APRSData.Buddie buddie, Dictionary<string, GeoPos> LastPositions)
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    if ((!pass) && (types[i].IndexOf("o") >= 0) && (pckt.StartsWith(";"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("i") >= 0) && (pckt.StartsWith(")"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("m") >= 0) && (pckt.StartsWith(":"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("q") >= 0) && (pckt.StartsWith("?"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("s") >= 0) && (pckt.StartsWith(">"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("t") >= 0) && (pckt.StartsWith("T#"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("u") >= 0) && (pckt.StartsWith("{"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("n") >= 0) && (pckt.StartsWith(":NWS"))) pass = true;
                    if ((!pass) && (types[i].IndexOf("w") >= 0) && (pckt.StartsWith("!") || pckt.StartsWith("#") || pckt.StartsWith("$") || pckt.StartsWith("*"))) pass = true;
                    if (pass && (!String.IsNullOrEmpty(calls[i])) && (buddie != null) && (!String.IsNullOrEmpty(buddie.name)))
                    {
                        if (buddie.name != calls[i]) pass = false;
                        if (pass && (!buddie.PositionIsValid)) pass = false;
                        if (pass && (LastPositions == null)) pass = false;
                        if (pass && !LastPositions.ContainsKey(buddie.name)) pass = false;
                        if (pass && (GeoPos.GetLengthKm(buddie.lat, buddie.lon, LastPositions[buddie.name].lat, LastPositions[buddie.name].lon, false) > dists[i])) pass = false;
                    };
                    if (pass) return true;
                };
                return false;
